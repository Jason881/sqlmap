...
def _setStdinPipeTargets():
    if not isinstance(conf.stdinPipe, _collections.Iterable):
        return
...
def _setPostprocessFunctions():
    ...
    for script in re.split(PARAMETER_SPLITTING_REGEX, conf.postprocess):
        ...
        try:
            ...
            if not os.path.exists(script):
                errMsg = f"postprocess script '{script}' does not exist"
                raise SqlmapFilePathException(errMsg)

            elif not script.endswith(".py"):
                errMsg = f"postprocess script '{script}' should have an extension '.py'"
                raise SqlmapSyntaxException(errMsg)
        except UnicodeDecodeError:
        ...
        infoMsg = f"loading postprocess module '{filename[:-3]}'"
        logger.info(infoMsg)

        if not os.path.exists(os.path.join(dirname, "__init__.py")):
            errMsg = "make sure that there is an empty file '__init__.py' "
            errMsg += f"inside of postprocess scripts directory '{dirname}'"
            raise SqlmapGenericException(errMsg)
        ...
        try:
            ...
            raise SqlmapSyntaxException(
                f"cannot import postprocess module '{getUnicode(filename[:-3])}' ({getSafeExString(ex)})"
            )
        ...
        if not found:
            ...
            errMsg += f"in postprocess script '{script}'"
            raise SqlmapGenericException(errMsg)
        else:
            try:
                _, _, _ = function("", {}, None)
